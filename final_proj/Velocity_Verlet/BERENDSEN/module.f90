...
do i=1,iter
        ...
        enddo
                CALL BERENDSEN_STAT(vhalf_x,vhalf_y,vhalf_z,T0,Ti)
        do j=1,atom
       ...
       enddo   
                ...
                T0=(Ek)*(eV/(kb*(3*real(atom)-1)))*10000*2
        ...
        enddo
                CALL BERENDSEN_STAT(vcx,vcy,vcz,T0,Ti)
        do j=1,atom
                vcx(j)=vhalf_x(j)+0.5*factor*(fx(j)/mass)*timestep
                vcy(j)=vhalf_y(j)+0.5*factor*(fy(j)/mass)*timestep
                vcz(j)=vhalf_z(j)+0.5*factor*(fz(j)/mass)*timestep
...
subroutine BERENDSEN_STAT(vcx,vcy,vcz,T0,Ti)
real*8,intent(in)::    T0,Ti
real*8,intent(inout):: vcx(:),vcy(:),vcz(:)
real*8::               lambda,t
integer::              i
t=0.8
lambda=(1+(timestep/t)*((Ti/T0)-1))**0.5
do i=1,atom
       vcx(i)=vcx(i)*lambda
       vcy(i)=vcy(i)*lambda
       vcz(i)=vcz(i)*lambda
